...
- **Token cap** (error if over the cap, warning if ≥`TOKEN_WARN_PCT`% of it) — token count is estimated as `byte_count / 3.5`. The cap is `NODE_CAP` (1,000) for every node except the project root's `CLAUDE.md`, which gets `ROOT_CAP` (2,000) — the root loads in every session and LCA placement funnels shared facts into it. The scan root only counts as the project root when `_is_project_root` confirms it: the target contains `.git`, or no ancestor directory up to the git/filesystem root contains a `CLAUDE.md`. This keeps the post-edit hook — which validates from the edited node's own directory — from inflating a child node's cap to 2,000, while still granting the root cap to a layer whose top node lives below the git root (e.g. a monorepo sub-tree). `TOKEN_WARN_PCT` (default 90) is a top-level constant that also drives the gradient red threshold, so both stay in sync. Both caps and the 3.5 ratio for English prose come from `skills/intent-layer/references/size-rules.md`. Offload files are uncapped.
